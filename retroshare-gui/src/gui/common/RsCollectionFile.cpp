...
bool RsCollectionFile::openNewColl(QWidget *parent) //const
{
	QString fileName;
	...
	connect(rcd,SIGNAL(saveColl(std::vector<ColFileInfo>)),this,SLOT(saveColl(std::vector<ColFileInfo>))) ;
	_saved=false;
	rcd->exec() ;

	return _saved;//save(fileName);
}
...
void RsCollectionFile::saveColl(std::vector<ColFileInfo> colFileInfos)
{
