...
//#else
    // if not Windows, then embedded emoticons only
...
void Emoticons::showSmileyWidget(QWidget *parent, QWidget *button, const char *slotAddMethod, bool above)
{
    ...
    // calculate the position of the widget to ??????????????????
    // oblicz rozmiar selektora
    // calculate the size selector
    ...
    //calculate distances from the widget to the left bank and the right
    int l_dist = pos.x();
