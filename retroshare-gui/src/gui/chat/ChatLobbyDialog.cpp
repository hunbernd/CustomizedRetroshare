...
void ChatLobbyDialog::addIncomingChatMsg(const ChatInfo& info)
{
	...
	if(!isParticipantMuted(name)) {
	  ui.chatWidget->addChatMsg(true, limitedname, sendTime, recvTime, message, ChatWidget::MSGTYPE_NORMAL);
...
void ChatLobbyDialog::displayLobbyEvent(int event_type, const QString& nickname, const QString& str)
{
	...
	case RS_CHAT_LOBBY_EVENT_PEER_LEFT:
        qsParticipant=str;
		ui.chatWidget->addChatMsg(true, tr("Lobby management"), QDateTime::currentDateTime(), QDateTime::currentDateTime(), tr("%1 has left the lobby.").arg(RsHtml::plainText(str).left(NICK_NAME_LIMIT)), ChatWidget::MSGTYPE_SYSTEM);
		emit peerLeft(id()) ;
		break;
	case RS_CHAT_LOBBY_EVENT_PEER_JOINED:
        qsParticipant=str;
		ui.chatWidget->addChatMsg(true, tr("Lobby management"), QDateTime::currentDateTime(), QDateTime::currentDateTime(), tr("%1 joined the lobby.").arg(RsHtml::plainText(str).left(NICK_NAME_LIMIT)), ChatWidget::MSGTYPE_SYSTEM);
		emit peerJoined(id()) ;
		break;
	...
	case RS_CHAT_LOBBY_EVENT_PEER_CHANGE_NICKNAME:
        qsParticipant=str;
		ui.chatWidget->addChatMsg(true, tr("Lobby management"), QDateTime::currentDateTime(), QDateTime::currentDateTime(), tr("%1 changed his name to: %2").arg(RsHtml::plainText(nickname).left(NICK_NAME_LIMIT), RsHtml::plainText(str).left(NICK_NAME_LIMIT)), ChatWidget::MSGTYPE_SYSTEM);
